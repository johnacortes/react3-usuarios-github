function ShowRepositories({ repos, allRepos, allRepositories }) {
  if (!repos) {
    return <div className="container-repositories">Repositories not fund</div>;
  }
  ...
  const updated = (updatedAt) => {
    ...
    const timeDifference = currentDate - updatedDate; // Diferencia en milisegundos
    const daysDifference = Math.floor(timeDifference / (1000 * 60 * 60 * 24)); // Convertir a días
    return daysDifference;
  };

  const renderRepos = repos.map((rep) => {
    return (
      <div className="repos" key={rep.id}>
        <div className="container-nameRepo"> {rep.name}</div>
        <div className="container-description">{rep.description}</div>
        <div className="container-puntuaciones">
          <div className="container-svgMit">{svgMit}MIT</div>
          <div className="container-forks">
            {svgNesting}
            {rep.forks}
          </div>
          <div className="container-stargazers">
            {svgStargazers}
            {rep.stargazers_count}
          </div>
          <div className="container-updated">
            Updated {updated(rep.updated_at)} days ago
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="container-repositories">
      ...
      <div className="container-button">
        <button className="button-all" onClick={allRepos}>
          {allRepositories ? "Preview" : "View all repositories"}
        </button>
      </div>
    </div>
  );
